#!/usr/bin/env python3
"""
Batch evaluation script for all Relative Clause fold models.
Runs evaluate_fold_rc.py on all available RC folds and aggregates the results.
"""
import argparse
import json
import re
...
def find_fold_models(output_dir: Path) -> List[Tuple[int, Path]]:
    """Find all RC fold model directories."""
    fold_models = []

    for model_dir in sorted(output_dir.glob("relative_clause_fold_*")):
        if model_dir.is_dir():
...
def evaluate_fold(
    ...
    cmd = [
        "uv", "run", "python", "evaluate_fold_rc.py",
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate all Relative Clause fold models"
    )
    parser.add_argument(
        "--models-dir",
        type=str,
        default="output/rc",
        help="Directory containing RC fold models",
    )
    parser.add_argument(
        "--config-template",
        type=str,
    ...
    parser.add_argument(
        "--summary-output",
        type=str,
        default="rc_all_folds_evaluation_summary.json",
    ...
    if not fold_models:
        print(f"No Relative Clause fold models found in {models_dir}")
        return

    print(f"Found {len(fold_models)} Relative Clause fold models")

    # Evaluate each fold
    all_results = {}

    for fold_num, checkpoint_path in tqdm(fold_models, desc="Evaluating RC folds"):
    ...
    # Aggregate Relative Clause results
    # RC has single construction: "RelativeClause"
    construction = "RelativeClause"
    ...
    # Print summary
    print("\n" + "="*60)
    print("RELATIVE CLAUSE EVALUATION SUMMARY")
    print("="*60)
