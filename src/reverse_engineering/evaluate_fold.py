#!/usr/bin/env python3
"""
Evaluation script for fold models.
Evaluates a trained fold checkpoint on garden-path effects and on the
self-paced reading corpora (Natural Stories, UCL, Smith 2013) via delta
log-likelihood.
"""
import argparse
import json
...
def evaluate_garden_path(
    ...
    selected_phenomena = training_kwargs.get(
        "selected_phenomena", ["MVRR", "NPS", "NPZ"]
    )
    ...
    logger.info("Evaluating garden-path with eval coefficients...")

    model.eval()
    with torch.no_grad():
        for batch in tqdm(eval_loader, desc="Eval Garden-Path", leave=False):
