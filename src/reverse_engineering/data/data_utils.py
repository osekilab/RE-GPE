...
def save_data_split_logs(
    ...
    # Check if this is a GardenPathDataset (doesn't have dataset.dataset structure)
    if not hasattr(train_dataset, 'dataset'):
        # For GardenPathDataset, skip the detailed logging for now
        print(f"Skipping data split logs for GardenPathDataset")
        return
