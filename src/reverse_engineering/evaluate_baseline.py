#!/usr/bin/env python3
"""
Evaluate untrained baseline models for comparison.
Mirrors evaluate_fold.py but loads models directly from HuggingFace instead of checkpoints.
"""
import argparse
import json
import logging
...
def evaluate_single_fold(args):
    ...
    test_path = folds_dir / f"fold_{args.fold_num}" / "garden_path_test.csv"
    train_path = folds_dir / f"fold_{args.fold_num}" / "garden_path_train.csv"
    ...
    # Load UNTRAINED model from HuggingFace (key difference from evaluate_fold.py)
    ...
    logger.info("\nEvaluating Garden-Path sentences...")
    garden_path_results = evaluate_garden_path(
        model=model,
        tokenizer=tokenizer,
    ...
    results["garden_path"] = garden_path_results
    logger.info(f"Garden-Path Loss: {garden_path_results['eval_loss']:.4f}")

    # Extract garden-path differences for easy access
    if "monitoring_data" in garden_path_results:
        gp_diffs = {}
        for construction in ["MVRR", "NPS", "NPZ"]:
            ...
            for roi in [0, 1, 2]:
                ...
                if key_actual in garden_path_results["monitoring_data"]:
                    gp_diffs[construction][f"roi{roi}"] = {
                        "actual": garden_path_results["monitoring_data"][key_actual],
                        "predicted": garden_path_results["monitoring_data"][key_predicted],
                        "n_pairs": garden_path_results["monitoring_data"].get(
    ...
    if "individual" in results:
        ...
        for dataset_name, dataset_data in results["individual"].items():
            if "delta_llh" in dataset_data and dataset_data["delta_llh"] is not None:
                print(f"  {dataset_name.upper()}: Delta LLH = {dataset_data['delta_llh']:.6f} "
    ...
    if "filler" in results:
        ...
        if "delta_llh" in results["filler"] and results["filler"]["delta_llh"] is not None:
            ...
            if "coefficients" in results["filler"]:
                print(f"  Surprisal coefficient: {results['filler']['coefficients'].get('beta_s', 0):.4f}")
...
def evaluate_all_folds(args):
    ...
    Note: For baseline models, only garden-path data varies across folds.
    ...
    # Determine if we need to evaluate non-garden-path datasets
    ...
    if evaluate_other_datasets:
        print("\nNote: Non-garden-path datasets will be evaluated only once")
    ...
    # Evaluate each fold
    all_results = {}
    shared_other_results = None  # Store results from non-garden-path datasets
    ...
    for idx, fold_num in enumerate(tqdm(valid_folds, desc="Evaluating baseline")):
        ...
        # For non-garden-path datasets, only evaluate on first fold
        # and reuse results for subsequent folds
        evaluate_others_this_fold = evaluate_other_datasets and (idx == 0 or shared_other_results is None)
        ...
        try:
            ...
            if result:
                ...
                if evaluate_others_this_fold and shared_other_results is None:
                    ...
                    print("  Stored non-garden-path results for reuse")
