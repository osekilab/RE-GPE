#!/usr/bin/env python3
"""
Train models on all filtered Relative Clause folds.

This script trains a model for each fold in the filtered RC folds directory,
using the relative_clause_template.yaml as the base configuration.
"""
...
def create_fold_config(base_config_path: Path, fold_idx: int, config_output_dir: Path, folds_dir: Path, model_output_base_dir: Path) -> Path:
    ...
    config['training_kwargs']['data_path'] = str(fold_path / "rc_train.csv")

    # Update evaluation data path
    config['training_kwargs']['eval_data_path'] = str(fold_path / "rc_test.csv")

    # Update output directory to include fold index
    config['training_kwargs']['output_dir'] = str(model_output_base_dir / f"relative_clause_fold_{fold_idx}")

    # Update experiment name to include fold
    config['experiment_name'] = f"relative_clause_roi_exclusion_fold_{fold_idx}"

    # Save fold-specific configuration
    fold_config_path = config_output_dir / f"relative_clause_fold_{fold_idx}.yaml"
...
def run_training(config_path: Path, fold_idx: int, dry_run: bool = False, device: str = None) -> bool:
    ...
    print(f"\n{'='*80}")
    print(f"Training Relative Clause fold {fold_idx}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train models on all filtered Relative Clause folds"
    )
    parser.add_argument(
        "--base-config",
        type=Path,
        default=Path("configs/relative_clause_template.yaml"),
        help="Base configuration file to use as template"
    )
    parser.add_argument(
        "--config-dir",
        type=Path,
        default=Path("configs/rc_fold_configs"),
    ...
    parser.add_argument(
        "--folds-dir",
        type=Path,
        default=Path("../../src/relative_clause_cross_validation/folds_filtered"),
    ...
    for i, fold_idx in enumerate(available_folds, 1):
        print(f"\n{'#'*80}")
        print(f"Processing Relative Clause fold {fold_idx} ({i}/{len(available_folds)})")
        print(f"{'#'*80}")
    ...
    print(f"\n{'='*80}")
    print("RELATIVE CLAUSE TRAINING SUMMARY")
    print(f"{'='*80}")
    ...
    if not args.dry_run:
        ...
        summary_file = Path("training_summary_rc.txt")
        with open(summary_file, 'w') as f:
            f.write(f"Relative Clause Training Summary - {start_time.strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"{'='*80}\n")
